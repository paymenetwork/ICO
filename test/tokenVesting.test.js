  ...
  describe("Vesting", function () {
    ...
    it("Should vest tokens gradually", async function () {
     ...
     // console.log(TGEOT1);
      ...
      await tokenVesting.setCurrentTime(TGEOT1 + 100 + duration);

      // check that the vested amount is 70, remember that 10 token is already claimed
      expect(
        await tokenVesting
          .connect(beneficiary1)
          .computeReleasableAmount(vestingScheduleId)
      ...
      await expect(
        tokenVesting.connect(beneficiary1).release(vestingScheduleId, 30)
      )
        .to.emit(testToken, "Transfer")
        .withArgs(tokenVesting.address, beneficiary1.address, 30);

      //another beneficiary cannot release token
      await expect(
        tokenVesting.connect(beneficiary2).release(vestingScheduleId, 30)
      )
      .to.be.revertedWith(
        "TokenVesting: only beneficiary and owner can release vested tokens"
      )
      ...
      await expect(tokenVesting.connect(owner).release(vestingScheduleId, 40))
      .to.emit(testToken, "Transfer")
      .withArgs(tokenVesting.address, beneficiary1.address, 40);
    ...
    //should release 20% of token at TGE
    await expect(tokenVesting.connect(beneficiary1).releaseForTGE(vestingScheduleId)
    ).to.emit(tokenVesting, "TokenReleasedAtTGE")
    .withArgs(beneficiary1.address, 20);

    //should revert if beneficiary tries a claim TGE token again
    await expect(tokenVesting.connect(beneficiary1).releaseForTGE(vestingScheduleId)
    ).to.be.revertedWith("TGE: Token Already claimed");

    //should revert if TGE time is not reached
    await tokenVesting.setCurrentTime(TGEOT1 - 1000);
    await expect(tokenVesting.connect(beneficiary1).releaseForTGE(vestingScheduleId)
    ).to.be.revertedWith("TGE: time not reached!");
